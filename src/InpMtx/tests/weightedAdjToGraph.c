...
/*
   -------------------------------------------------------------
   read in an adjacency structure file and create a Graph object
...
int       *adjncy, *offsets, *vadj, *vwghts ;
Graph     *graph ;
FILE      *fp, *msgFile ;
...
if ( msglvl > 3 ) {
   fprintf(msgFile, "\n vertex weights") ;
   IVfp80(msgFile, nvtx, vwghts, 80, &ierr) ;
   fflush(msgFile) ;
}
/*
   -------------------
...
graph->type = 1 ;
graph->totvwght = IVsum(nvtx, vwghts) ;
graph->vwghts = vwghts ;
if ( msglvl > 0 ) {
...
if ( msglvl > 0 ) {
   ...
   fprintf(msgFile, "\n vertex weights in ascending order") ;
   IVqsortUp(nvtx, vwghts) ;
   IVfp80(msgFile, nvtx, vwghts, 80, &ierr) ;
}
/*
   ---------------------------------
