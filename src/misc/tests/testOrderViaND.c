...
/*
   ----------------------------------------------
   order a graph via a nested dissection ordering
...
fprintf(msgFile, 
        ...
        maxsize, maxzeros, seed, outETreeFileName) ;
fflush(msgFile) ;
/*
   ------------------------
...
/*
   ---------------------------------------
   order the graph using nested dissection
   ---------------------------------------
*/
MARKTIME(t1) ;
frontETree = orderViaND(graph, maxdomainsize, seed, msglvl, msgFile) ;
MARKTIME(t2) ;
fprintf(msgFile, "\n\n CPU %9.2f : order the graph via ND", t2 - t1) ;
if ( msglvl > 2 ) {
