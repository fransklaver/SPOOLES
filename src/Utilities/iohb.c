...
static int ParseRfmt(char* fmt, int* perline, int* width, 
              int* prec, int* flag) ;
static char* substr(const char* S, const int pos, const int len);
...
/*
    ...
    char Ptrfmt[17], Indfmt[17], Valfmt[21], Rhsfmt[21];
*/
    char *Title   = CVinit(73, '\0') ;
    char *Key     = CVinit(9, '\0') ;
    char *Type    = CVinit(4, '\0') ;
    char *Rhstype = CVinit(4, '\0') ;
    char *Ptrfmt = CVinit(17, '\0') ;
    char *Indfmt = CVinit(17, '\0') ;
    char *Valfmt = CVinit(21, '\0') ;
    char *Rhsfmt = CVinit(21, '\0') ;
...
/*
       fprintf(stdout, 
               ...
               "\n Valprec = %d" 
               "\n Valflag = %c",
               Valfmt, Valperline, Valwidth, Valprec, Valflag) ;
*/
    }
...
/* commented out by cca, new version below */
/*
static int ParseIfmt(char* fmt, int* perline, int* width)
{
*/
...
/*
   --------------------------------------------------------------
   purpose -- to fill *pperline with the number of int's per line
              and *pwidth the width of the field
...
char   *locI, *locLeftParen, *locRightParen ;

upcase(format);
locI          = strchr(format, 'I') ;
locRightParen = strchr(format, ')') ;
if ( (locLeftParen = strchr(format, '(')) == NULL ) {
   return(-1) ;
}
if ( (locI = strchr(format, 'I')) == NULL ) {
   return(-2) ;
}
if ( (locRightParen = strchr(format, ')')) == NULL ) {
   return(-3) ;
}
*locI = '\0' ;
...
static int ParseRfmt(char* fmt, int* perline, int* width, int* prec, int* flag)
{
/*************************************************/
...
/* start cca mods */
{ 
    char *locLeftParen  = strchr(fmt, '(') ;
    char *locRightParen = strchr(fmt, ')') ;
    char *locFlag       = strchr(fmt, *flag) ;
    char *locDot        = strchr(fmt, '.') ;
